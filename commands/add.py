...
class Add(BaseCommand):
    ...
    async def handle(self, params, message, client):
        ...
        msg = "Please enter the description of the script\n"
        await channel.send(msg)
        log += msg

        description = await client.wait_for("message", check=check, timeout=600.0)
        log += f"<@!{message.author.id}>: {description.content}\n"
        ...
        if reaction[0].emoji == "✅":
            response = requests.post(f"{settings.DB_URL}/hub/add", data=json_code)
            ...
            await channel.send("Congratulations your script has been accepted to be apart of the script hub. Give up to 24 hours to be update on the website")
